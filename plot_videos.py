...
def plot_video(joints,
    ...
    for (j, frame_joints) in enumerate(joints):
        ...
        # frame_joints_2d = np.reshape(frame_joints, (50, 3))
        frame_joints_2d = np.reshape(frame_joints[:], (50, 3))
        # Draw the frame given 2D joints
        ...
        # ref_joints_2d = np.reshape(ref_joints[], (50, 3))
        ref_joints_2d = np.reshape(ref_joints[:], (50, 3))
        # Draw the frame given 2D joints
    ...
    for (j, frame_joints) in enumerate(joints):
        ...
        # Draw the frame given 2D joints
        draw_frame_2D(frame, frame_joints_2d * 3)
        ...
        if references is not None:
            ...
            draw_frame_2D(ref_frame, ref_joints_2d * 3)
...
def alter_DTW_timing(pred_seq, ref_seq):
    ...
    # Run DTW on the reference and predicted sequence在参考和预测序列上运行DTW
    d, cost_matrix, acc_cost_matrix, path = dtw(ref_seq[:, :], pred_seq[:, :], dist=euclidean_norm)

    # Normalise the dtw cost by sequence length按序列长度标准化dtw成本
